...
export default function ActivityListItem({id, verb, subject, object, balance, currency}: ActivityItemProps) {
    return (
        ...
        >
            ...
            <div className="grow text-sm  text-slate-800">
                {verb === "updated" && (
                ...
                {balance > 0 ? (
                    <p className="text-slate-500 font-normal mt-1"> You Recieved {" "}
                        <span className="text-green-700 font-medium">{currency} {balance}</span></p>
                ) : (
                    <p className="text-slate-500 font-normal mt-1"> You borrowed {" "}
                        <span className="text-rose-700 font-medium"> {currency}{balance}</span>
                    </p>
                )}
                <p className="text-slate-400 font-normal mt-1 text-xs">Yesterday, 22:30</p>
            </div>
        </Link>
    );
}
