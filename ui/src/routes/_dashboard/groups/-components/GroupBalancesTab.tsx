...
export function GroupBalancesTab({ group_uid }: GroupBalancesTabProps) {
  const {
    ...
    error: groupError,
  } = useApiQuery(ApiRoutes.GROUP_DETAIL, {
    group_uid,
  });
  const {
    data: balances,
    ...
    error: balancesError,
  } = useApiQuery(ApiRoutes.GROUP_OUTSTANDING_BALANCE, { group_uid });
  const { currentUser } = useAuth();

  if (groupPending || balancesPending || !currentUser) {
    return (
  ...
  const error = groupError ?? balancesError;
