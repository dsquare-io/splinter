...
function RootComponent() {
    ...
    async function deleteComment(comment_uid: string) {
        return confirm({
            ...
            description: (
                <>
                    Are you sure you want to delete <span className="font-medium text-gray-800">Comment</span>{' '}
    ...
    return (
        <div className="h-screen flex flex-col justify-between">
            <div>
                <div
                    className=
                        'relative grid gap-x-5 border-b border-gray-900/5 px-4 pb-6 pt-10 sm:px-6 md:px-8 justify-start items-center'
                >
                    <div
            ...
            <div className="my-3 px-4 sm:px-6 md:px-8">
                <div className="flex flex-col justify-between gap-6 inset-y-0 justify-items-start">
                    <div className="-space-y-px">
                        {Object.entries(groupBy(commentsData?.results ?? [], (comment) => {
                            ...
                            .map(([date, comments]) => (
                                ...
                                >
                                    <div
                                        className="sticky top-[96px]  px-6 py-1 text-sm font-medium text-gray-500 text-center">
                                        <h3 className="uppercase">{formatDate(date)}</h3>
                                    </div>
                                    <div className="flex flex-col justify-start gap-8">
                                        {comments.map((comment) => (
                                            <div key={comment.uid} className="flex gap-2 w-full items-start">
                                                <Avatar
                                                    className="size-8 bg-white"
                                                    fallback={comment.user.fullName}
                                                />
                                                <div className="w-max space-y-1 bg-gray-100 p-2 rounded-md">
                    ...
                    <Form
                        ...
                        <TextFormField name="comment" className="grow" aria-label="comment">
                            <div className="relative">
                                <Input
                                    type="text"
                                    placeholder="Add a comment"
                                />
                                <Button size="small" variant="plain" slot={null}
                        ...
                        <Button size="small" variant="plain" type="submit" slot="submit">
